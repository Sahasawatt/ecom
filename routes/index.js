...
router.get("/", async function (req, res, next) {
  ...
  try {
    ...
    sql = 'SELECT * FROM tb_group_product ORDER BY name DESC';

    let[groupProducts, feildsGroupProduct] = await conn.query(sql);
...
router.post("/changeProfile", isLogin, (req, res) => {
  ...
  conn.query(sql, params, (err, result) => {
    if (err) throw err;
    req.session.message = "Save Successfully";
...
router.get("/product", isLogin, (req, res) => {
  let sql =
    "SELECT tb_product.* , tb_group_product.name AS group_product_name FROM tb_product LEFT JOIN tb_group_product ON tb_group_product.id = tb_product.group_product_id ORDER BY tb_product.id DESC";
  conn.query(sql, (err, result) => {
...
router.post("/addProduct", isLogin, (req, res) => {
  ...
  form.parse(req, (err, fields, file) => {
    ...
    let newPath =
      "C://Users/Legion/Documents/Kob_course/workshop-ecom/app/public/images/";
    newPath += file.img.originalFilename;

    fs.copyFile(filePath, newPath, () => {
      let sql =
        "INSERT INTO tb_product(group_product_id, barcode , name ,cost, price ,img) VALUES(? , ? ,? ,? ,? , ?)";
      let params = [
...
router.get("/editProduct/:id", isLogin, (req, res) => {
  ...
  let params = req.params.id;
  conn.query(sql, params, (err, product) => {
    if (err) throw err;
    let sql = "SELECT * FROM tb_group_product ORDER BY name";
...
router.post("/editProduct/:id", isLogin, (req, res) => {
  ...
  form.parse(req, (err, fields, file) => {
    ...
    fs.copyFile(filePath, pathUpload, () => {
      let sqlSelect = "SELECT * FROM tb_group_product WHERE id = ?";
      ...
      conn.query(sqlSelect, paramSelect, (err, oldProduct) => {
        ...
        let sql =
          "UPDATE tb_product SET group_product_id=? , barcode=? , name=?, cost=?, price=?, img=? WHERE id =?";
        let params = [
          fields["group_product_id"],
          fields["barcode"],
          fields["name"],
...
router.get("/mycart", async (req, res) => {
  ...
  if (cart.length > 0) {
    for (let i = 0; i < cart.length; i++) {
      ...
      let [rows, feilds] = await conn.query(sql, params);
      let product = rows[0];
      let p = {
        qty: c.qty,
        id: product.id,
...
router.post("/confirmOrder", async (req, res) => {
  let conn = require("./connect2");
  let sql =
    "INSERT INTO tb_order(name, address, phone, created_date)VALUES(?, ?, ?, NOW())";
...
router.get("/confirmOrderSuccess", function (req, res) {
...
router.get("/orderInfo/:id", isLogin, (req, res) => {
  let sql =
    "SELECT tb_order_detail.*, tb_product.barcode, tb_product.name, tb_product.img FROM tb_order_detail LEFT JOIN tb_product ON tb_product.id = tb_order_detail.product_id WHERE tb_order_detail.order_id = 5 ORDER BY tb_order_detail.order_id DESC;";
  let params = [req.params.id];
...
router.get('/reportSalePerDay' , isLogin , async(req, res) =>{
  ...
  for(let i = 1; i <= daysInMonth; i++){
    let sql = 'SELECT SUM(qty * price) AS totalPrice FROM tb_order_detail LEFT JOIN tb_order ON tb_order.id = tb_order_detail.order_id WHERE DAY(tb_order.pay_date) = ? AND MONTH(tb_order.pay_date)= ? AND YEAR(tb_order.pay_date) = ?';
    let params= [i, m, y];
    let [rows, feilds] =await conn.query(sql, params);
  ...
  res.render('reportSalePerDay' , {arr: arr , y: y, m: m , arrYear: arrYear, arrMonth: arrMonth});
});
...
router.get('/reportSalePerMonth', isLogin , async (req, res) => {
  ...
  for(let i = 1; i < 12 ; i++){
    let sql = 'SELECT SUM(qty * price) AS totalPrice FROM tb_order_detail LEFT JOIN tb_order ON tb_order.id = tb_order_detail.order_id WHERE MONTH(tb_order.pay_date)= ? AND YEAR(tb_order.pay_date) = ?';
    let params = [i, y];
    let [rows, fields] = await conn.query(sql , params);
    arr.push(rows[0].totalPrice);
  }
...
router.get('/reportSalePerProduct' , isLogin , async (req, res) => {
  ...
  for(let i = 0; i < rows.length; i++){
    ...
    sql += ' WHERE tb_order_detail.product_id = ? AND tb_order.pay_date IS NOT NULL';

    let [rows2, feild2] = await conn.query(sql , [id]);
...
router.get('/importToStock' , isLogin, (req, res) => {
  let sql ='SELECT tb_product.barcode, tb_product.name, tb_stock_in.qty, tb_stock_in.create_date, tb_stock_in.id, tb_stock_in.remark'
  sql += ' FROM tb_stock_in';
  sql += ' LEFT JOIN tb_product ON tb_stock_in.product_id = tb_product.id'

  conn.query(sql ,(err, results) => {
    if(err) throw err;
    res.render('importToStock' , {stockIns: results});
  });
});
...
router.post('/importToStock', isLogin , async(req, res) => {
  ...
  try{
    ...
    if(product.length > 0){
      let id = product[0].id;
      sql = 'INSERT INTO tb_stock_in(product_id, qty, create_date, remark) VALUES(? , ? ,NOW() , ? )';
      params = [
        id,
        req.body['qty'],
        req.body['remark']
...
router.get('/exportFromStock' , isLogin, (req, res) => {
  let sql ='SELECT tb_product.barcode, tb_product.name, tb_stock_out.qty, tb_stock_out.create_date, tb_stock_out.id, tb_stock_out.remark'
  sql += ' FROM tb_stock_out';
  sql += ' LEFT JOIN tb_product ON tb_stock_out.product_id = tb_product.id'

  conn.query(sql ,(err, results) => {
    if(err) throw err;
    res.render('exportFromStock' , {stockOuts: results});
  });
});
...
router.post('/exportFromStock', isLogin, async (req, res) => {
  ...
  try{
    ...
    if(product.length > 0){
      let id = product[0].id;
      sql = 'INSERT INTO tb_stock_out(product_id, qty, create_date, remark) VALUES(? , ? ,NOW() , ? )';
      params = [
        id,
        req.body['qty'],
        req.body['remark']
...
router.get('/reportStock' , isLogin ,async (req, res) => {
  ...
  try {
    ...
    for(let i =0 ; i < products.length ; i++) {
      ...
      let [stockIn, fieldsIn] = await conn.query(sql ,params);

      sql = 'SELECT SUM(qty) AS qtyOut FROM tb_stock_out WHERE product_id = ?'
      let [stockOut, fieldsOut] = await conn.query(sql ,params);
